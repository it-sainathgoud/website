...
export async function generateStaticParams() {
  const allLegal = getLegalPages();

  return allLegal.map((post) => ({
    slug: post.slug,
  }));
}
...
export async function generateMetadata(
  ...
  const legal = getLegalPages().find((post) => post.slug === params.slug);

  if (!legal) {
    return;
  }

  const { title, summary: description } = legal.metadata;
...
export default async function SingleLegal(
  props: {
    params: Promise<{ slug: string }>;
  }
) {
  const params = await props.params;
  const legal = getLegalPages().find((legal) => legal.slug === params.slug);
